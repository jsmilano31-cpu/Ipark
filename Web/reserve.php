...
try {
    ...
    $stmt = $conn->query("SELECT * FROM parking_slots WHERE status = 'Vacant' ORDER BY floor_number, slot_number");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        ");
        $stmt->execute([$parking_slot_id, $end_time, $start_time, $end_time, $start_time, $start_time, $end_time]);
        ...
        if ($stmt->fetchColumn() > 0) {
            ...
            // Create reservation
            $stmt = $conn->prepare("
                INSERT INTO reservations (user_id, parking_slot_id, car_type, plate_number, start_time, end_time)
                VALUES (?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([$_SESSION['user_id'], $parking_slot_id, $car_type, $plate_number, $start_time, $end_time]);
            
            // Update parking slot status
            $stmt = $conn->prepare("UPDATE parking_slots SET status = 'Reserved' WHERE id = ?");
            $stmt->execute([$parking_slot_id]);
