...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($stmt->fetch()) {
            ...
            $stmt = $conn->prepare("INSERT INTO parking_slots (slot_number, floor_number, status) VALUES (?, ?, ?)");
            $stmt->execute([$slot_number, $floor_number, $status]);
...
try {
    $stmt = $conn->query("SELECT * FROM parking_slots ORDER BY floor_number, slot_number");
    $parking_slots = $stmt->fetchAll();
} catch(PDOException $e) {
