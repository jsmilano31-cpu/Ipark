...
try {
    // Total parking slots
    $stmt = $conn->query("SELECT COUNT(*) as total_slots FROM parking_slots");
    $totalSlots = $stmt->fetch()['total_slots'];

    // Available slots
    $stmt = $conn->query("SELECT COUNT(*) as available_slots FROM parking_slots WHERE status = 'Vacant'");
    $availableSlots = $stmt->fetch()['available_slots'];
    ...
    // Active reservations
    $stmt = $conn->query("SELECT COUNT(*) as active_reservations FROM reservations WHERE status = 'Confirmed'");
    $activeReservations = $stmt->fetch()['active_reservations'];
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            try {
                                ...
                                echo "<tr><td colspan='6' style='text-align: center; color: var(--gray-500); padding: 2rem;'>Error loading recent reservations</td></tr>";
                            }
                            ?>
                        </tbody>
                    </table>
